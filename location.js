  ...
  document.getElementById('loc-list').innerHTML=filtered.map(i=>{
    ...
    return`<div class="loc-zone-card">
      <div class="loc-zone-title">
        ...
        <button class="btn btn-purple btn-sm" onclick="openLocModalFor(${i.id})">+ 庫位</button>
      </div>
      ${locs.length?locs.map(l=>`
        <div class="loc-slot">
          ...
          <button class="btn btn-danger btn-sm" style="margin-left:10px;" onclick="removeLoc(${i.id},'${l.zone}',event)">移除</button>
        </div>`).join('')
...
function saveLoc(){
  const itemId=parseInt(document.getElementById('loc-item').value);
  ...
  if(existing){
    if(qty===0){locations=locations.filter(l=>!(l.itemId===itemId&&l.zone===zone));toast(`庫位 ${zone} 已清空`,'warn');}
    else{existing.qty=qty;toast(`庫位 ${zone} 已更新`,'success');}
  }else{
    if(qty>0){locations.push({itemId,zone,qty});toast(`庫位 ${zone} 已設定`,'success');}
  }
  closeModal('modal-loc');renderLoc();saveData();
}
